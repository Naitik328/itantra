...
Be honest about the payoff: it is small. Measured on en_IN-female-medium with
num_threads=4 and a typical ~100-byte sentence:

    generation   420 ms -> 389 ms   (~7%)
    model load  1509 ms -> 1436 ms

It is free and safe, so it is worth doing, but the real latency wins are
num_threads and not reloading the model per utterance (see INTEGRATION.md).
...
Usage:
    python 8_optimize_onnx.py en_IN-female-medium.onnx
"""
import os
import shutil
import sys
...
src = sys.argv[1] if len(sys.argv) > 1 else "en_IN-female-medium.onnx"
tmp = src + ".opt.tmp"
