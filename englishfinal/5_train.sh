...
# Delete the stale cache from the previous (mixed-gender, 12,000-clip) run
# before retraining on the female-only set. Entries for the 3,490 dropped male
# clips are never read again, but they occupy ~7GB on an 80GB volume that has
# already crashed training with "Disk quota exceeded" more than once.
rm -rf /workspace/piper_work/en_cache/
